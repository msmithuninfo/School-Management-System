                ...
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="form-group col-md-3">
                      ...
                      <a href="{{ url('backend/teacher/attendance/student') }}" class="btn btn-success" style="margin-top: 30px;">Reset</a>
                    </div>
                  </div>
                </div>
              </form>
                                    ...
                                    @endphp
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <label style="margin-right: 8px;">
                                                  <input value="3" type="radio" {{ ($attendance_type == '3') ? 'checked' : '' }} id="{{ $value->id }}" class="SaveAttendance" name="attendance{{ $value->id }}"> Half Day 
                                                </label>
                                                <label>
                                                  <input value="4" type="radio" {{ ($attendance_type == '4') ? 'checked' : '' }} id="{{ $value->id }}" class="SaveAttendance" name="attendance{{ $value->id }}"> Absent 
                                                </label>
                                            </td>
                                        </tr>
                                    @endforeach
...
<script type="text/javascript">
  $('.SaveAttendance').change(function(e){
    ...
    $.ajax({
        type: "POST",
        url: "{{ url('backend/teacher/attendance/student/save') }}",
        data : {
          "_token": "{{ csrf_token() }}",
