                            ...
                            <form name="post" class="SubmitForm">
                              ...
                              <tr>
                                ...
                                <td>
                                  ...
                                  <a href="{{ url('backend/teacher/my_exam_result/print?exam_id='.Request::get('exam_id').'&student_id='.$student->id) }}" target="_blank" class="btn-sm float-right btn btn-primary">Print</a>
                                  ...
                                  {{-- <span>
                                    @php
                                      $totalSubject = $getSubject->count();
                                      $totalSubjectMark = 100 * $totalSubject;
                                      $totalgrading = $totalSubjectMark - (20 * $totalSubject);
                                      $APlus = $totalgrading;
                                      $AGrade = $totalgrading - 10;
                                      $AMinus = $totalgrading - 20;
                                      $BPlus = $totalgrading - 30;
                                      $BGrade = $totalgrading - 40;
                                      $BMinus = $totalgrading - 50;
                                    @endphp
                                    @if ($totalStudentMark >= $APlus)
                                      <b style="color: green;">A+</b>
                                    @elseif ($totalStudentMark >= $AGrade)
                                      <b style="color: green;">A</b>
                                    @elseif ($totalStudentMark >= $AMinus)
                                      <b style="color: green;">A-</b>
                                    @elseif ($totalStudentMark >= $BPlus)
                                      <b style="color: green;">B+</b>
                                    @elseif ($totalStudentMark >= $BGrade)
                                      <b style="color: green;">B</b>
                                    @elseif ($totalStudentMark >= $BMinus)
                                      <b style="color: green;">B-</b>
                                    @endif
                                  </span> --}}
...
<script type="text/javascript">
  ...
  e.preventDefault();
      $.ajax({
        type: "POST",
        url: "{{ url('backend/teacher/submit_marks_register') }}",
        data : $(this).serialize(),
        dataType : "json",
  ...
  $('.SaveSingleSubject').click(function(e){
    ...
    $.ajax({
        type: "POST",
        url: "{{ url('backend/teacher/single_submit_marks_register') }}",
        data : {
          "_token": "{{ csrf_token() }}",
          id : id,
